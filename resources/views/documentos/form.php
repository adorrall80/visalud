<header class="page-heading page-heading--form"><div><a class="back-link" href="/documentos">← Volver a documentos</a><p class="eyebrow">Archivo privado</p><h1>Cargar documento</h1><p class="muted">El archivo quedará protegido y disponible únicamente para tu familia.</p></div></header>

<?php if(!empty($errors['general'][0])):?><div class="alert alert--error"><?= $view->escape($errors['general'][0]) ?></div><?php endif; ?>

<form class="record-form" method="post" action="/documentos" enctype="multipart/form-data">
    <?= $view->csrfField() ?>
    <section class="form-section"><div class="form-section__heading"><span>1</span><div><h2>Archivo</h2><p>PDF o imagen de hasta <?= $view->escape($maxUploadMb) ?> MB.</p></div></div><label class="upload-zone"><input type="file" name="archivo" accept=".pdf,.jpg,.jpeg,.png,.webp,application/pdf,image/jpeg,image/png,image/webp" required><strong>Seleccionar documento</strong><span>PDF, JPG, PNG o WEBP</span></label></section>
    ...
    <div class="form-actions"><a class="button button--secondary" href="/documentos">Cancelar</a><button class="button button--primary" type="submit">Guardar documento</button></div>
</form>
