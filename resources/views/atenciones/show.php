...
<dialog class="appointment-modal ai-prompt-modal" id="ai-prompt-modal" aria-labelledby="ai-prompt-title">
    ...
    <div class="appointment-modal__body"><textarea class="ai-prompt-text" id="ai-prompt-text" readonly><?= $view->escape($promptIa) ?></textarea><p class="copy-feedback" data-copy-text-feedback="ai-prompt-text" aria-live="polite"></p></div>
    <footer class="appointment-modal__footer"><form method="dialog"><button class="button button--secondary" type="submit">Cerrar</button></form><button class="button button--primary" type="button" data-copy-text="ai-prompt-text">Copiar texto</button></footer>
</dialog>
...
<section class="document-list attention-document-list"><?php foreach($documentos as $documento): ?>
    <article class="document-row">
        ...
        <div class="document-row__body"><div><span class="badge"><?= $view->escape($documento['tipo_nombre']) ?></span></div><h2><?= $view->escape($documento['nombre']) ?></h2><p><?= $view->escape($documento['fecha_documento'] ?: substr($documento['created_at'],0,10)) ?> · Subido por <?= $view->escape($documento['subido_por_nombre']) ?><?= $documento['descripcion'] ? ' · ' . $view->escape($documento['descripcion']) : '' ?></p></div>
        <div class="document-row__actions"><a class="button button--compact button--secondary" href="/documentos/<?= $view->escape($documento['id']) ?>/edit">Editar</a><a class="button button--compact button--secondary" href="/documentos/<?= $view->escape($documento['id']) ?>/download">Descargar</a><?php if(!empty($documento['puede_eliminar'])): ?><form method="post" action="/documentos/<?= $view->escape($documento['id']) ?>" onsubmit="return confirm('¿Eliminar este documento de forma permanente?')"><?= $view->csrfField() ?><input type="hidden" name="_method" value="DELETE"><button class="button button--compact button--danger" type="submit">Eliminar</button></form><?php endif; ?></div>
    </article>
